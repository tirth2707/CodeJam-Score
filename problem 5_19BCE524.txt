...
public class Solution{
    ...
    void solve(int TC) throws Exception {
        int n = ni(), k = ni();
        int[][] mat = new int[n][n];
        if(generateLatinSquare(mat, n, k, 0, 0)){
            pn("Case #"+TC+": POSSIBLE");
            for(int[] j:mat){
                for(int i:j)p(i+" ");pn("");
            }
        }
        else pn("Case #"+TC+": IMPOSSIBLE");
    }
    boolean generateLatinSquare(int[][] mat, int n, int k, int r, int c){
        if(r == n && c == 0){
            int s = 0;
        ...
        for(int nxt = 1; nxt <= n; nxt++){
            boolean f = true;
            for(int j = 0; j< r; j++)if(mat[j][c] == nxt)f = false;
            for(int j = 0; j< c; j++)if(mat[r][j] == nxt)f = false;
            if(f){
                mat[r][c] = nxt;
                if(generateLatinSquare(mat, n, k, r+(c+1)/n, (c+1)%n))return true;
            }
        }
        return false;
    }
